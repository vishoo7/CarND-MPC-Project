...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // set reference velocity so as to motivate the car to advance and not just minimize cte and epsi error
    const double ref_v = 75;

    // set tunable weights for each value
    const int cte_weight = 2500;
    const int epsi_weight = 2500;
    const int v_weight = 1;
    const int delta_and_a_weight = 500;
    const int delta_weight = 5;
    const int a_weight = 5;
    const int delta_delta_weight = 200;
    const int a_delta_weight = 10;
    ...
    for (int i = 0; i < N - 1; i++) {
      fg[0] += delta_and_a_weight * CppAD::pow(vars[delta_start + i] * vars[v_start+i], 2);
    ...
    for (int i = 0; i < N - 2; i++) {
      fg[0] += delta_delta_weight * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += a_delta_weight * CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }
